...
public class Board {
	...
	static void setPiece(int x, int y, Piece piece) {
		if (piece != null) {
			piece.setX(x);
			piece.setY(y);
		}
	...
	static boolean isPathClear(int x1, int y1, int x2, int y2) {
    ...
    static String chooseCoin(String pos, Color color) {

        if ( pos.equals("exit") == true  ) {
            System.exit(0);
        }
        if ( pos.equals("Print") ) {
            printBoard();
		...
		if ( pos.length() == 9 && pos.substring(3, 9).equals("--help") ) {
			String coordinates = pos.substring(0, 2);
			if (coordinates.length() != 2) {
				System.out.println("Invalid Tile please try again");
				return "";
			}

			ArrayList<String> infos =  isSafe( coordinates, color );
			if ( infos.size()==0 ) {
        ...
        Piece currPiece = getPiece(file, rank);
        if ( currPiece == null ) {
			...
			if ( viables.size() == 0 ) {
				...
				for( String i: viables ) {
					System.out.print(i+", ");
				}
			}
			System.out.println();

            return currPiece.getName();
        }
		...
		// if (piece.equals("castle")) {
		 if (piece.equals("king")) {
			...
			if ( king.color == Color.WHITE ) {
				if ( splitStr[1].equals("g1")  ) {
					return king.castle("K");
				} else if ( splitStr[1].equals("c1") ) {
					return king.castle("Q");
				}
			} else {

				if ( splitStr[1].equals("g8")  ) {
					return king.castle("K");
				} else if ( splitStr[1].equals("c8") ) {
					return king.castle("Q");
				}
			}
		}
